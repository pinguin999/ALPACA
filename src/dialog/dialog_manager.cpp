...
void DialogManager::play(const std::string& dialogName, std::optional<sol::function> callback) {
    ...
    if (auto _game = game.lock()) {
        if (!choiceTexts.empty()) {
            ...
            for (auto text = choiceTexts.begin(); text != choiceTexts.end(); text++) {
                if (pointer_position.y > pos - BOX_PADDING && pointer_position.y < pos + BOX_HEIGHT - BOX_PADDING) {
...
void DialogManager::selectCurrentAnswer(int index) {
    if (currentAnswers == nullptr || currentDialog == nullptr ||
        index < 0) {
        return;
    }
